...
void Level4::computerMove(Board* board, TextDisplay* td, GraphDisplay* gd) {
                    ...
                    for (int newCol = 0; newCol < 8; ++newCol) {
                        if (board->isMoveable(row, col, newRow, newCol, board) && 
                            ...
                            // Check move and ensure it's safe
                                     ...
                                     isSafeAfterMove(board, row, col, newRow, newCol) &&
                                     !isRepeatedCheck(row, col, newRow, newCol)) {
    ...
    if (!bestMoves.empty()) {
        ...
        if (board->willCheckOpponent(fromRow, fromCol, toRow, toCol)) {
            if (recentChecks.size() >= maxRecentChecks) {
                recentChecks.pop_front();
            }
            recentChecks.push_back(std::make_tuple(fromRow, fromCol, toRow, toCol));
        }
...
bool Level4::isRepeatedCheck(int fromRow, int fromCol, int toRow, int toCol) {
    for (const auto& move : recentChecks) {
        if (std::get<0>(move) == fromRow && std::get<1>(move) == fromCol &&
            std::get<2>(move) == toRow && std::get<3>(move) == toCol) {
            return true;
        }
    }
    return false;
